...
public class Deeplearning4jService {
	...
	private static final int INPUT_FEATURES = 265;
	...
	private void buildModel() {
		MultiLayerConfiguration config = new NeuralNetConfiguration.Builder().seed(12345)
				.optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT)
				.updater(new Adam.Builder().learningRate(0.001).build()).list()
				...
				.layer(0,
						new OutputLayer.Builder(LossFunctions.LossFunction.MSE).nOut(1).activation(Activation.IDENTITY)
								.build())
	...
	public double predict(double[] deepLearningInputParam) {
		double[] inputFeatures = deepLearningInputParam;
